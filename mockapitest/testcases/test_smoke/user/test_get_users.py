...
def test_exact_search_user():
    """获取系统中的用户列表精确检索"""
    request_body = apis.GetUsersApiUsersGetAPI.QueryParams(offset=0, limit=10, username="李四")
    ...
    assert response.response_model.message == "success"
    for info in response.response_model.data:
        assert info.username is not None and "李四" in info.username
