...
def test_login_with_invalid_inputs(invalid_credentials):
    ...
    assert 400 <= response.cached_response.raw_response.status_code < 500, f"Unexpected status code for invalid input {invalid_credentials}. Status: {response.cached_response.raw_response.status_code}"
...
@pytest.mark.auth2
def test_login_with_valid_format_but_wrong_credential(valid_format_credentials):
    ...
    # 断言：应为未授权状态
    assert response.cached_response.raw_response.status_code == 401, f"Expected 401 for wrong credential, got {response.cached_response.raw_response.status_code} for {valid_format_credentials}"
    # 可以进一步断言响应体中包含错误信息
...
@pytest.mark.auth2
def test_login_success_with_hypothesis_variation(password_suffix):
    """示例：结合已知有效用户名和hypothesis生成的密码变体进行测试。"""
    # 假设已知一个有效用户名 "test_user"
    valid_username = "aomaker"
    ...
    # 预期失败
    assert response.cached_response.raw_response.status_code != 200, "Should not login successfully with random password variation"
